...
class Lattice {
public:
  using LatticeSU3 = std::vector<SU3Matrix>;

  int index(int x, int y, int z, int t, int mu) {
    return (((x * Ns + y) * Ns + z) * Nt + t) * dir + mu;
  }

  Lattice() : U(std::vector<SU3Matrix>(N)){};

  ~Lattice() = default;

  SU3Matrix &operator()(int i) { return U[i]; }
