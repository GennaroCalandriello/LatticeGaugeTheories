...
void Heatbath::HB_update(Lattice &U, const double beta) {
  ...
  for (int idx = 0; idx < N; idx++) {
    ...
    if (a != 0) {
      ...
      SU3Matrix::Matrix R_elems = {
          {{{r_(0, 0), r_(0, 1), 0}}, {{r_(1, 0), r_(1, 1), 0}}, {{0, 0, 1}}}};
      SU3Matrix R(R_elems);

      W = R * W;

      SU2Matrix s_ = heatbath_subgroup(W, beta, "s");
      SU3Matrix::Matrix S_elems = {
          {{{s_(0, 0), 0, s_(0, 1)}}, {{0, 1, 0}}, {{s_(1, 0), 0, s_(1, 1)}}}};
      SU3Matrix S(S_elems);

      W = S * W;

      SU2Matrix t_ = heatbath_subgroup(W, beta, "t");
      SU3Matrix::Matrix T_elems = {
          {{{1, 0, 0}}, {{0, t_(0, 0), t_(0, 1)}}, {{0, t_(1, 0), t_(1, 1)}}}};
      SU3Matrix T(T_elems);
      ...
      U(U.index(x, y, z, t, mu)) = su3_generator();
      cout << "a = 0" << endl;
    }
  }
}
...
SU3Matrix Heatbath::staple(Lattice U, int x, int y, int z, int t, int mu) {
  ...
  for (int nu = 0; nu < dir; nu++) {
    ...
    if (mu != nu) {
      ...
      A += U(U.index((x + a_mu[0]) % Ns, (y + a_mu[1]) % Ns, (z + a_mu[2]) % Ns,
                     (t + a_mu[3]) % Nt, nu)) *
